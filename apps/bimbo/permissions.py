...
def _rows_to_agencias(rows):
    """Convierte filas de agencias_bimbo al formato estándar de retorno."""
    return [
        {
            "id": r["id"],
            "CEVE": r["CEVE"],
...
def get_agencias_permitidas(user):
    ...
    Prioridad para usuarios no-superadmin:
      1. PermisoBimboAgente — asignaciones explícitas por CEVE (nuevo sistema)
      2. conf_empresas → agencias_bimbo.db_powerbi — fallback para usuarios
         sin asignaciones explícitas (compatibilidad con el sistema anterior)

    Para superusuarios: todas las agencias activas en agencias_bimbo.
    """
    from apps.permisos.models import ConfEmpresas

    # ── Superusuario: todas las agencias activas ──────────────────────────────
            ...
            with engine.connect() as conn:
                rows = conn.execute(
                    text(
                        ...
                        f"FROM agencias_bimbo WHERE db_powerbi IN ({placeholders}) ORDER BY CEVE"
                    ),
                    params,
                ).mappings().all()
            return _rows_to_agencias(rows)
        except Exception as exc:
            logger.error("Error consultando agencias_bimbo (superuser): %s", exc)
            return []

    # ── Usuarios normales: PermisoBimboAgente primero ─────────────────────────
    agencia_ids_asignados = list(
        PermisoBimboAgente.objects.filter(user=user).values_list("agencia_id", flat=True)
    )

    if agencia_ids_asignados:
        # El usuario tiene asignaciones explícitas por CEVE — usarlas
        try:
            engine = _get_bimbo_engine()
            placeholders = ", ".join(f":id_{i}" for i in range(len(agencia_ids_asignados)))
            params = {f"id_{i}": v for i, v in enumerate(agencia_ids_asignados)}
            with engine.connect() as conn:
                rows = conn.execute(
                    text(
                        "SELECT id, CEVE, Nombre, nmOficinaV, db_powerbi, estado "
                        f"FROM agencias_bimbo WHERE id IN ({placeholders}) ORDER BY CEVE"
                    ),
                    params,
                ).mappings().all()
            return _rows_to_agencias(rows)
        except Exception as exc:
            logger.error("Error consultando agencias_bimbo por IDs asignados: %s", exc)
            # Si falla la BD, continúa al fallback de conf_empresas

    # ── Fallback: conf_empresas → agencias_bimbo.db_powerbi ──────────────────
    try:
    ...
    try:
        ...
        logger.error("Error consultando agencias_bimbo (fallback conf_empresas): %s", exc)
        # Fallback final: usar datos de ConfEmpresas si tiene ceve poblado
        agencias = []
        for e in empresas:
            if e.ceve:
                agencias.append({
