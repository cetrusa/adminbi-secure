...
class ExtraeBiExtractor:
    ...
    def _resolve_read_chunk_size(self) -> int:
        # Lectura puede ser más agresiva porque no construye SQL de INSERT.
        return self._resolve_chunk_size(fallback=10000, minimum=1000, maximum=200000)

    def _resolve_insert_chunk_size(self) -> int:
        # Inserción mantiene tope conservador para evitar paquetes SQL gigantes.
        return self._resolve_chunk_size(fallback=5000, minimum=1000, maximum=50000)
    ...
    def insertar_sql(
        ...
        CHUNK_THRESHOLD = effective_chunk_size
        ...
        logging.info(
            f"Insertar_sql finalizado para {self.txTabla}: {len(resultado_out):,} filas preparadas en {elapsed:.2f}s."
        )
